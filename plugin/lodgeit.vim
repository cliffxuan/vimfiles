" lodgeit.vim: Vim plugin for newmandistrib:Ewt9Gleb@paste.newmanonline.org.uk
...
srv = ServerProxy('http://newmandistrib:Ewt9Gleb@paste.newmanonline.org.uk/xmlrpc/', allow_none=True)
...
def paste_id_from_url(url):
    regex = re.compile(r'^http://newmandistrib:Ewt9Gleb@paste.newmanonline.org.uk/show/([^/]+)/?$')
    m = regex.match(url)
    if m is not None:
        return m.group(1)
...
# new paste or reply
    ...
    url = 'http://newmandistrib:Ewt9Gleb@paste.newmanonline.org.uk/show/%s' % paste_id
