...
do
    ...
    function VibeChat.cycle_focus()
        if not VibeChat.state.layout_active then return end
    ...
    function VibeChat.open_chat_window()
        if VibeChat.state.layout_active and VibeChat.state.input_win_id and vim.api.nvim_win_is_valid(VibeChat.state.input_win_id) then
        ...
        vim.bo[VibeChat.state.output_buf_id].filetype, vim.bo[VibeChat.state.output_buf_id].modifiable, vim.bo[VibeChat.state.output_buf_id].buflisted = 'markdown', false, false
        ...
        vim.keymap.set('i', '<C-s>', function() VibeChat.send_message() end, create_keymap_opts(VibeChat.state.input_buf_id))
        vim.keymap.set('n', '<CR>', function() VibeChat.send_message() end, create_keymap_opts(VibeChat.state.input_buf_id))
        ...
        for _, buf_id in ipairs({VibeChat.state.input_buf_id, VibeChat.state.output_buf_id, VibeChat.state.context_buf_id}) do
            ...
            vim.keymap.set('n', '<Tab>', function() VibeChat.cycle_focus() end, create_keymap_opts(buf_id))
            vim.keymap.set('n', '<S-Tab>', function() VibeChat.cycle_focus_reverse() end, create_keymap_opts(buf_id))
        end
    ...
    function VibeChat.send_message()
        if VibeChat.state.is_thinking then return end
        ...
        local messages_for_api = {{ role = "system", content = "You are a helpful coding assistant. Use the provided file contexts to answer the user's prompt."}}
        for _, file_path in ipairs(VibeChat.state.context_files) do
            local content = table.concat(vim.fn.readfile(file_path), '\n')
            local context_msg = "CONTEXT from file `" .. vim.fn.fnamemodify(file_path, ':p') .. "`:\n\n```\n" .. content .. "\n```"
...
do
    function VibeDiff.get_last_ai_response()
        ...
        if not last_ai_message then return nil end
...
-- User Commands
-- =============================================================================
vim.api.nvim_create_user_command('VibeChat', VibeChat.open_chat_window, { desc = 'Open the Vibe Coding sidebar layout' })
vim.api.nvim_create_user_command('VibeClose', VibeChat.close_layout, { desc = 'Close the Vibe Coding sidebar' })
vim.api.nvim_create_user_command('VibeAddToContext', VibeChat.add_context_to_chat, { desc = 'Add a file to the Vibe chat context using fd' })
vim.api.nvim_create_user_command('VibeDiff', VibeDiff.show, { desc = 'Diff the last AI code block with the main editor buffer' })
