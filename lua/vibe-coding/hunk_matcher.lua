...
function HunkMatcher:_apply_single_hunk(hunk, hunk_idx, modified_lines)
  ...
  if found_at > -1 then
    ...
    if adjusted_search_lines ~= search_replace.search_lines then
      adjusted_search_replace.replacement_lines = self:_rebuild_replacement_lines_for_fuzzy_match(
        hunk, adjusted_search_lines, search_replace.to_add
      )
    end
...
function HunkMatcher:_rebuild_replacement_lines_for_fuzzy_match(hunk, adjusted_search_lines, to_add)
  ...
  -- Now build replacement lines by starting with adjusted search lines
  -- and inserting additions at the right positions
  for i, line in ipairs(adjusted_search_lines) do
    table.insert(replacement_lines, line)
    
    -- Insert any additions that should come after this line
    for _, addition in ipairs(addition_lines) do
      if addition.position == i then
        table.insert(replacement_lines, addition.text)
      end
    end
  end
...
function HunkMatcher:_generate_result_message()
  ...
  elseif self.applied_hunks > 0 then
    local success_msg = string.format('Partially applied %d/%d hunks to %s', self.applied_hunks, total_hunks, target_file)
    if self.split_hunks then
      success_msg = success_msg .. '\nFailed hunks: ' .. table.concat(
        vim.tbl_map(function(f) return '#' .. f.index end, self.failed_hunks), ', '
      )
    end
